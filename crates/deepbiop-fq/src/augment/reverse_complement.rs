...
impl ReverseComplement {
    /// Apply reverse complement to a batch of sequences in parallel.
    ///
    /// # Arguments
    ///
    /// * `sequences` - Slice of sequences to transform
    ///
    /// # Returns
    ///
    /// Vector of reverse complemented sequences
    ///
    /// # Example
    ///
    /// ```
    ...
    /// let results = rc.apply_batch(&sequences);
    /// assert_eq!(results.len(), 2);
    /// ```
    pub fn apply_batch(&mut self, sequences: &[Vec<u8>]) -> Vec<Vec<u8>> {
        use rayon::prelude::*;
...
mod tests {
    ...
    fn test_batch_processing() {
        let mut rc = ReverseComplement::new();
        let sequences = vec![b"ACGT".to_vec(), b"TTAA".to_vec(), b"GGCC".to_vec()];

        let results = rc.apply_batch(&sequences);

        assert_eq!(results.len(), 3);
        assert_eq!(results[0], b"ACGT"); // Palindrome
        assert_eq!(results[1], b"TTAA"); // Palindrome
        assert_eq!(results[2], b"GGCC"); // Palindrome
    }
}
